#!/usr/bin/python3
"""
Module sends a search request to the twitter Api
"""
...
if __name__ == "__main__":
    ...
    a_headers = {'Authorization': 'Basic {}'.format(b64_key),
            'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'}

    a_data = {'grant_type': 'client_credentials'}

    a_resp = requests.post(a_url, headers=a_headers, data=a_data)

    access_token = a_resp.json().get('access_token')

    s_headers = {'Authorization': 'Bearer {}'.format(access_token)}

    s_params = {'q': sys.argv[3], 'result_type': 'recent', 'count': 5}
    ...
    for x in tweet_data['statuses']:
        print("[{}] {} by {}".format(x.get('id'),
            x.get('text'), x.get('user').get('name')))
